...
RSpec.describe '2019/03/Part1' do
    part1 = Y2019::Day3::Part1
    it 'can parse an instruction' do
        expect(part1.parse_instruction('R17')).to eq({direction: 'R', length: 17})
    end
    ...
    it 'can build paths' do
        ...
        expect(part1.build_path(instructions)).to eq expected
    end

    it 'finds intersections' do
        a = ['R8','U5','L5','D3'].map { |item| part1.parse_instruction item}
        b = ['U7','R6','D4','L4'].map { |item| part1.parse_instruction item}
        path_a = part1.build_path a
        path_b = part1.build_path b
        expect(part1.find_intersections(path_a, path_b)). to eq Set[{x: 3, y: 3}, {x: 6, y: 5}]
    end

    it 'finds the closest intersection' do
        intersections = Set[{x: 3, y: 3}, {x: 6, y: 5}]
        expect(part1.closest_intersection(intersections)). to eq({x: 3, y: 3})
    end
    ...
    it 'can calculate the closest intersection distance directly' do
        ...
        result = part1.harness(instructions_a, instructions_b)
