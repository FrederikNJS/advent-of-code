        ...
        while true
            ...
            instances.each.with_index do |instance, index|
                unless(instance.out.empty?)
                    ...
                    x = instance.out.pop
                    y = instance.out.pop
                    #puts "packet from :#{index} ===> :#{dest} |> x: #{x}, y: #{y}"
                    if dest == 255
                        nat = [x, y]
                    else
                        instances[dest].in << x
            ...
            if !any_outputs && !nat.nil?
                if empty_count > 10
                    #puts "empty"
                    if prev_y == nat[1]
                        puts "First y delivered twice in a row: #{nat[1]}"
                        exit(0)
                    else
                        prev_y = nat[1]
                    end

                    puts "Sending message from nat to 0: #{nat[1]}"

                    instances[0].in << nat[0]
                    instances[0].in << nat[1]
                    nat = nil
                else
                    empty_count += 1
                end
