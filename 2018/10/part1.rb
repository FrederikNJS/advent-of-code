...
bounds = calc_bounds lights, time
dimensions = calc_dimensions bounds
sky = Array.new(dimensions[:y] + 1) { Array.new(dimensions[:x] + 1, '.') }

lights.each do |light|
  sky[y_calc(light, time) - bounds[:y_min]][x_calc(light, time) - bounds[:x_min]] = '#'
end
