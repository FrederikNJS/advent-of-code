...
if __FILE__ == $0
    ...
    xes = hull.keys.map { |location| location.x }
    ys = hull.keys.map { |location| location.y}
    puts "x = [#{xes.min}, #{xes.max}]"
    puts "y = [#{ys.min}, #{ys.max}]"

    canvas = Array.new(ys.max - ys.min + 1) { |y| Array.new(xes.max - xes.min + 1) { |x| '.'}}
    hull.each { |location, color| canvas[location.y - ys.min][location.x - xes.min] = color == 1 ? '#' : ' ' }
    canvas.reverse.each { |row| puts row.join }
end
